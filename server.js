...
//// database is a JavaScript object containg all of the stored model instances. 
//// The users and articles properties contain JavaScript objects which will contain all of your created users and articles.
//// Each of these objects will have keys representing the ID of the saved resource and values of the whole resource.
//// For example if we created an article with ID 1, the articles object would look like this {1: {id: 1, url: 'article url', ...}}. 
//// Additionally the database object contains a property for keeping track of the ID of the next article to create (to ensure no two articles have the same ID)
...
//// The keys of this object are the path of the request, and the values are objects containing all of the possible HTTP verbs for that path.
//// Each of those verbs map to a function to call if that route is hit and will be passed the path of the request and the request object (containing the request body)
